...
replay_limits limits_value(const json & value, bool active) {
    ...
    require(result.cold_capacity_bytes != 0 && result.hot_capacity_slots != 0,
        "cache replay capacity must be nonzero");
...
public:
    hierarchy_replay(
        ...
        profile(requested_profile), events(requested_events), candidates(requested_candidates),
        ...
        require(slots != 0 && slots <= INT32_MAX && limits.hot_capacity_slots <= INT32_MAX,
    ...
    json run() {
        ...
        for (size_t token = 0; token < events.size(); ++token) {
            for (const auto & layer : events[token].at("layers")) {
                ...
                consume_demands(token, layer_id, layer.at("experts").get<std::vector<int32_t>>());
        ...
        for (const auto & item : entries) if (item.second.origin == "SPECULATIVE") remaining.push_back(item.first);
...
private:
    ...
    void discard(key_type key, const std::string & reason) {
        ...
        const replay_entry entry = found->second;
        entries.erase(found);
        if (entry.origin != "SPECULATIVE") return;
        std::string outcome;
    ...
    bool demand_victim(bool cold, key_type & result) const {
        ...
        for (const auto & item : entries) {
            const int32_t slot = cold ? item.second.cold_slot : item.second.hot_slot;
            if (slot < 0) continue;
            const auto order = std::make_tuple(item.second.last_touch, item.second.layer, item.second.expert);
            if (!found || order < best) {
                found = true;
                best = order;
                result = item.first;
    ...
    std::pair<int32_t, int32_t> ensure_demand_capacity(uint64_t token, uint64_t physical) {
        ...
        return {cold_slot, free_slot(false)};
    }
    ...
    bool speculative_victims(
        ...
        bool cold_free = free_slot(true) >= 0 && !require_cold_victim;
        bool hot_free = !needs_hot || (free_slot(false) >= 0 && !require_hot_victim);
        std::vector<std::pair<key_type, replay_entry>> available;
        for (const auto & item : entries) {
            if (item.second.origin == "SPECULATIVE" && !excluded.count(item.first)) available.push_back(item);
        }
    ...
    void submit_batch(const std::vector<json> & requested) {
        ...
        for (const auto & item : entries) {
            if (item.second.origin != "SPECULATIVE" || item.second.phase == "READY") continue;
            in_flight++;
        ...
        for (const auto & candidate : requested) {
            ...
            std::string reason;
            if (entries.count(key)) reason = "target_ready_or_higher_priority";
            else if (in_flight >= limits.max_speculative_flights) reason = "flight_budget";
            else if (physical > limits.max_speculative_storage_bytes_in_flight - storage_in_flight) {
            ...
            } else if (physical > limits.max_speculative_storage_bytes_per_token - token_storage[token]) {
            ...
            for (const auto & item : entries) {
                if (item.second.origin != "SPECULATIVE") continue;
                cold_speculative++;
            ...
            const bool admitted = !reason.empty() ? false : speculative_victims(physical,
                readiness == "DEVICE_READY", cold_speculative >= limits.max_speculative_cold_slots,
            ...
            const std::string phase = cost.predictor_compute_ns >= cost.lead_ns ? "QUEUED" :
                (cost.predictor_compute_ns + cost.speculative_service_ns > cost.lead_ns ? "SUBMITTED" : "READY");
            entries[key] = {key.first, key.second, payload, physical, cold_slot, hot_slot,
                ...
                candidate.at("flight_ordinal"), clock, phase};
            accepted.push_back(key);
            in_flight++;
            storage_in_flight += physical;
            h2d_in_flight += h2d;
            token_storage[token] += physical;
            token_h2d[token] += h2d;
            counters.accepted++;
            const uint64_t submitted_storage = phase == "QUEUED" ? 0 : physical;
            ...
            emit(actions, { {"type", "ENQUEUE"}, {"flight_ordinal", candidate.at("flight_ordinal")},
                ...
                {"priority", candidate.at("trigger") == "ROUTER_RESULT" ? "PREFETCH_NEXT" : "PREFETCH_SPECULATIVE"},
                {"storage_bytes", physical}, {"h2d_bytes", h2d},
    ...
    void resolve_deadline(uint64_t token, int32_t layer) {
        ...
        for (const auto & item : entries) {
            if (item.second.origin == "SPECULATIVE" && item.second.deadline_token == token &&
    ...
    void seed() {
        ...
        require(!profile.seed.empty() && seed_bytes <= limits.cold_capacity_bytes &&
            profile.seed.size() <= limits.hot_capacity_slots && profile.seed.size() <= cold_slot_capacity,
        ...
        for (const auto & item : profile.seed) {
            ...
            entries[key] = {item.layer, item.expert, item.payload_bytes, item.physical_bytes,
                cold_slot, hot_slot, "STATIC_SEED", 0, item.layer, item.count, -1, clock, "READY"};
            emit(actions, { {"type", "SEED_LOAD"}, {"flight_ordinal", -1}, {"token", 0},
                {"layer", item.layer}, {"expert", item.expert}, {"cold_slot", cold_slot},
                {"hot_slot", hot_slot}, {"storage_bytes", item.physical_bytes}, {"h2d_bytes", item.payload_bytes} });
        }
...
json replay(const json & input) {
    require_fields(input, {"schema_version", "profile_path", "policy", "transport", "readiness", "temporal_window_tokens",
    ...
    const auto limits = limits_value(input.at("limits"), policy != LLAMA_EXPERT_PREFETCH_POLICY_OFF);
