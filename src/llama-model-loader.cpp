...
llama_model_loader::llama_model_loader(
    ...
    if (!fname.empty()) {
        ...
        source_files.push_back({fname, files.back()->size(), gguf_get_alignment(metadata), true});
        ...
        if (n_split > 1) {
            ...
            for (idx = 1; idx < n_split; idx++) {
                ...
                source_files.push_back({fname_split, files.back()->size(), gguf_get_alignment(ctx_gguf.get()), true});
                contexts.emplace_back(ctx);
        ...
        source_files.push_back({"", files.back()->size(), gguf_get_alignment(metadata), false});
        contexts.emplace_back(ctx);
